...
A result must show the user *why* it matched, with the matched span marked.
Match priority is:
...
Highlight offsets are relative to the returned snippet, not to the page,
which is what the web UI renders.

The page text index is passed in explicitly. Snippet generation reads a
coherent snapshot captured by the caller, so a concurrent mutation cannot
tear a result set.
"""
