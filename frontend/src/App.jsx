...
const RECENT_SEARCHES_STORAGE_KEY =
  'domain-specific-search-recent-searches'
...
function saveRecentSearch(queryValue) {
...
function buildQuerySuggestions(inputValue, recentValues) {
  const normalizedInput = inputValue.trim().toLowerCase()

  if (!normalizedInput) {
    return recentValues.slice(0, 5)
  }
...
function App() {
  ...
  const handleRecentSearch = (recentQuery) => {
    const normalized = recentQuery.trim()
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Keep document-selection state independent from upload state.
  ...
  const toggleDocumentSelection = (filename) => {
    if (!filename || isMutating || isSearching) {
      return
    }
  ...
  const toggleSelectVisibleResults = () => {
    if (
      isMutating ||
      isSearching ||
      results.length === 0
    ...
    const visibleFilenames = results
      .map((result) => result?.title)
      .filter(Boolean)

    const allSelected = visibleFilenames.every(
      (filename) =>
  ...
  const handleBulkDelete = async () => {
    const filenames = [...selectedDocuments]
    ...
    const confirmed = window.confirm(
      `Delete ${filenames.length} selected document${filenames.length === 1 ? '' : 's'
      } from the search index and data folder?`,
    )
    ...
    setSearchError('')
    setUploadMessage(
      `Deleting ${filenames.length} document${filenames.length === 1 ? '' : 's'
      }...`,
    )

    // Invalidate active search responses.
    searchAbortControllerRef.current?.abort()
    searchRequestIdRef.current += 1

    try {
      const response = await fetch(
        `${API_BASE_URL}/api/documents/bulk-delete`,
        {
          method: 'POST',
          headers: {
      ...
      setUploadMessage(
        summaryParts.length > 0
          ? `Bulk delete complete: ${summaryParts.join(', ')}.`
          : (
            data.message ||
    ...
    <div className="container">
        ...
        >
          ...
          <span>
            {indexingStatus.state === 'ERROR'
              ...
              : isMutating
                ? 'Updating the search index... Search remains available.'
                : 'Indexing documents... Search remains available on the last READY snapshot.'}
          </span>
        </div>
      )}
      ...
      {hasSearched && !isSearching && !searchError && (
        <>
          <div className="results-toolbar">
            <div className="metrics">
              ...
              {indexingStatus.state === 'READY' ? (
                ...
                <> | Index: UPDATING · Search: READY SNAPSHOT</>
              )}
            </div>
          ...
          {results.length > 0 && (
            <div className="results-selection-toolbar">
